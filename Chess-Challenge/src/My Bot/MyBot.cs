...
public class MyBot : IChessBot
{
    ...
    private struct BoardNode
    {
        ...
        {
            return new BoardNode(_isWhiteToMove, Value, IsSearchDone, BestMoves.ToHashSet(), Transitions.ToDictionary(t => t.Key, t => t.Value));
        }
    }
        ...
        while (_searchQueue.Count > 0)
        {
            ...
            try
            {
                ...
                foreach (var previewMove in board.GetLegalMoves())
                {
                    ...
                    if (!parentBoardNode.Transitions.TryAdd(previewMove, board.ZobristKey) && parentBoardNode.Transitions[previewMove] != board.ZobristKey)
                    {
                        Console.WriteLine("Shit thing happen!");
                        parentBoardNode.Transitions[previewMove] = board.ZobristKey;
                    }
